...
interface ImageViewerProps {
  ...
  onIndexChange?: (index: number) => void; // Optional callback when index changes
}
...
export function ImageViewer({
  ...
  // Sync currentIndexState with currentIndex prop
  useEffect(() => {
  ...
  }, [isOpen, hasMultipleImages, currentIndexState]);
  ...
  const handlePrevious = () => {
    const newIndex = currentIndexState > 0 ? currentIndexState - 1 : images.length - 1;
  ...
  const handleNext = () => {
    const newIndex = currentIndexState < images.length - 1 ? currentIndexState + 1 : 0;
    setCurrentIndexState(newIndex);
    if (onIndexChange) {
      onIndexChange(newIndex);
    }
  ...
  const handleDownload = () => {
    if (currentImage) {
      ...
      link.download = `image-${currentIndexState + 1}.jpg`;
              ...
              <div className="flex items-center gap-2">
                {hasMultipleImages && (
                  <span className="text-sm text-white/90">
                    {currentIndexState + 1} of {images.length}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-2">
